...
func GetSessionById(sessionId uuid.UUID) (*models.Session, error) {
	database, err := db.CreateDatabase()
	...
	stmt, err := database.Prepare("SELECT Id, StartTime, EndTime, UserId, IsDeleted, SessionState FROM sessions WHERE Id = ?;")
	...
	err = stmt.QueryRow(sessionId).Scan(&session.Id, &session.StartTime, &session.EndTime, &session.UserId, &session.IsDeleted, &session.SessionState)
	if err != nil {
		return nil, err
	}
